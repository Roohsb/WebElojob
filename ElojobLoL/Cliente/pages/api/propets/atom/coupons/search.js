...
export default async function(req,res){
   ...
   * Verificando Token
   */
  if(typeof req.body.token === 'undefined' || typeof req.body.coupon === 'undefined' || checkUser === 0)
  {
       res.json({
         status:false,
         error: 1})
  ...
  try{
  const Coupon = await Mysql.prepareQuery("SELECT * FROM booster_cupons WHERE id = ?",[req.body.coupon])
