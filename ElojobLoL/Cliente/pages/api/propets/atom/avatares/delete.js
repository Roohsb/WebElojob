...
export default async function(req,res){
   ...
   * Verificando Token
   */
  if(typeof req.body.token === 'undefined' ||  typeof req.body.avatar === 'undefined' ||checkUser === 0)
  {
       res.json({
         status:false,
         error: 1})
  ...
  try{
   await Mysql.prepareQuery("UPDATE elo_users_personalization SET avatar = ? WHERE avatar = ?",[1,req.body.avatar])
   await Mysql.prepareQuery("DELETE FROM personalization_avatar WHERE id = ?",[req.body.avatar])
