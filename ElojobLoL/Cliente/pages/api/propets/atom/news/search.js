...
export default async function(req,res){
   ...
   * Verificando Token
   */
  if(typeof req.body.token === 'undefined' || typeof req.body.news === 'undefined' || checkUser === 0)
  {
       res.json({
         status:false,
         error: 1})
  ...
  try{
  const News = await Mysql.prepareQuery("SELECT * FROM updates_published WHERE id = ?",[req.body.news])
