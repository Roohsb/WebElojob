...
export default async function(req,res){
   ...
   * Verificando Token
   */
  if(typeof req.body.token === 'undefined' ||  typeof req.body.banner === 'undefined' ||checkUser === 0)
  {
       res.json({
         status:false,
         error: 1})
  ...
  try{
   await Mysql.prepareQuery("UPDATE elo_users_personalization SET avatar = ? WHERE banner = ?",[1,req.body.banner])
   await Mysql.prepareQuery("DELETE FROM personalization_banner WHERE id = ?",[req.body.banner])
