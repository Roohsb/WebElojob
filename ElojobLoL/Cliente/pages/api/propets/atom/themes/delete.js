...
export default async function(req,res){
   ...
   * Verificando Token
   */
  if(typeof req.body.token === 'undefined' ||  typeof req.body.theme === 'undefined' ||checkUser === 0)
  {
       res.json({
         status:false,
         error: 1})
  ...
  try{
   await Mysql.prepareQuery("UPDATE elo_users_personalization SET thema = ? WHERE thema = ?",[1,req.body.theme])
   await Mysql.prepareQuery("DELETE FROM personalization_themes WHERE id = ?",[req.body.theme])
