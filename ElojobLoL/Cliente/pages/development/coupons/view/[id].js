...
export async function getServerSideProps(ctx) {
  ...
  const coupon = typeof ctx.req.__NEXT_INIT_QUERY.id !== 'undefined' ? ctx.req.__NEXT_INIT_QUERY.id:  ctx.req.url.split("/")[4]

  const couponDetails = await LoggedAuthM.couponsSearch(cookies._AuthorizationJobx,coupon)
  
  if(couponDetails.status === false){

   ctx.res.writeHead(302, { Location: '/development/coupons/list' });
   ctx.res.end();
   return {
     props: {
       cookies,
               ...
               <div className="row">
                  <div className="col">
                     <div className="card-profile-stats d-flex justify-content-center">
                        <div>
                           <span className="heading">{cxt.final.cp.Coupon[0].code}</span>
                           <span className="description">Cupom</span>
                        </div>
                     </div>
                  </div>
               </div>
               <div className="pl-lg-4"><div className="row">
                  <div className="col-lg-6"><div className="form-group"><label className="form-control-label" for="input-username">Codigo</label>

                  <input type="text" id="input-codigo-coupon" className="form-control" defaultValue={cxt.final.cp.Coupon[0].code} placeholder="Codigo"/>
                  
                  </div>
                  </div>
                  <div className="col-lg-6"><div className="form-group">
                     <label className="form-control-label" for="input-email">ID</label>
                     <input type="email" id="input-id-coupon" className="form-control" defaultValue={cxt.final.cp.Coupon[0].id} placeholder="id" disabled/>
                     
                     </div></div></div>
                    <div className="row">
                     <div className="col-lg-6"><div className="form-group">
                     <label className="form-control-label" for="input-email">Desconto %</label>
                     <input type="number" id="input-desconto-coupon" className="form-control" defaultValue={cxt.final.cp.Coupon[0].discount} placeholder="id"/>
                     
                     </div></div>
                    
                     <div className="col-lg-6"><div className="form-group">
                     <label className="form-control-label" for="input-email">Expiracao</label>
                     <input type="text" id="input-expiracao-coupon" className="form-control" defaultValue={cxt.final.cp.Coupon[0].expires} placeholder="id"/>
                     
                     </div></div></div>


                     <div className="header pb-6 d-flex align-items-center">
                     <button type="button" className="btn btn-success" onClick={(e) => LoggedAuthM.atualizarCoupon(cxt.final.c._AuthorizationJobx)}>Salvar Alterações</button>
                     <button type="button" className="btn btn-danger"  data-toggle="modal" data-target="#modal-delete-banner">Apagar Cupom</button></div>
   ...
   <div className="modal fade" id="modal-delete-banner" tabindex="-1" role="dialog" aria-labelledby="modal-delete-banner" aria-hidden="true">
        ...
        <div className="modal-content">
            ...
            <div className="modal-body">
                ...
                <p>Você deseja realmente apagar o Cupom <strong id="inserirusername">{cxt.final.cp.Coupon[0].code}</strong></p>
                <p>Ao continuar, você deve estar ciente que, <strong style={{color: 'red', fontSize: '13px'}}>NÃO SERA POSSÍVEL REVERTER ESTÁ AÇÃO</strong></p><p>Este cupom sera apagado eternamente</p>
            </div>
            <div className="modal-footer">
                <button onClick={(e) => LoggedAuthM.deleteCoupon(cxt.final.c._AuthorizationJobx)}type="button"  className="btn btn-primary">Continuar</button>
