...
class EloBooster extends Calculator
{
    ...
    public function startBooster()
    {
        ...
        return array(
            ...
            "SelecetDivision" => $this->currentDivision,
            "ToElo" => $this->desiredElo,
            "ToDivision" => $this->desiredDivision,
            "Service" => $this->typeCalculate,
            "status" => true,
            "Url" => "http://localhost:81/user/servico/comprar"
        );
    }
...
class Calculator_Coach{
    ...
    public function __construct($options)
    {
        ...
        $this->Theamount = $options['qtdaulas'];
    }
    ...
    protected function securityTesting()
    {
        if(!$this->necessaryTools()->CheckLevelCoach($this->Curso) ||
            !is_numeric($this->Theamount) || $this->Theamount > 10 || 
            $this->Theamount == 0)
            {
                throw new MyException(3);
            }

            if (!is_array($this->DEFAULT_JOBX) || count($this->DEFAULT_JOBX) < 3)
            {
                $error = !is_array($this->DEFAULT_JOBX) ?? 69;
                $error = $error != 69 ? 2 : 1;
                throw new MyException($error);
    ...
    public function calculateCoach()
    {
        $this->securityTesting();
        foreach($this->DEFAULT_JOBX as $Coach){
            if($Coach["level"] == $this->Curso){
                $this->Value += $Coach["value"] * $this->Theamount;
                break;
            }
        }
    }
...
class Coach extends Calculator_Coach
{

    protected $DEFAULT_JOBX = Essential::DEFAULT_JOBX_COACH;

    public function startCoach()
    {
        $this->calculateCoach();
        return array(
            "total" => $this->Value,
            "curso" => $this->Curso,
            "quantidade" => $this->Theamount,
            "status" => true,
            "Url" => "http://localhost:81/user/coach/contratar"
        );
    }
